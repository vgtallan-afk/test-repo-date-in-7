export default async function handler(req, res) {
  ...
  let apiKey = String(process.env.GROQ_API_KEY || "")
    .trim()
    .replace(/^Bearer\s+/i, "")
    .replace(/[\r\n\t ]+/g, "");
  ...
  if (!apiKey.startsWith("gsk_")) return res.status(500).json({ error: "Invalid GROQ_API_KEY format. Use only the key starting with gsk_, without Bearer, quotes, spaces, or line breaks." });
...
Use only browser-safe fonts. Do not ask the user to download fonts. Browser-safe options: ${safeFonts.join(" | ")}.`;
    ...
    const system = `You are the safe AI editor inside HTML Editor, a visual HTML landing page editor.
...
Prefer safe, small operations. You can create carousel sections using clean HTML/CSS only, with horizontal overflow, scroll-snap, and image slides. Do not use scripts.
...
- Keep landing pages fast, clean, and mobile-friendly.`;
    ...
    if (referenceImage.startsWith("data:image/")) {
      ...
      messages = [
        { role: "system", content: system + "\nIf a reference screenshot is provided, use it only as visual inspiration for layout, hierarchy, spacing, colors, and structure. Do not copy logos or copyrighted text from the screenshot unless the user explicitly supplied that content." },
        { role: "user", content: [
          { type: "text", text: "User request and editor context JSON:\n" + JSON.stringify(user) },
    ...
    const parsed = await callGroq(messages, modelToUse, 4500);
